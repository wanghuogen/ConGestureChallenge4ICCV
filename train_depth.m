...
ff_train2 = fopen('Image4LSTM/train/train_depth_full.txt','w');
ff_val2 = fopen('Image4LSTM/train/val_depth_full.txt','w');

while feof(fpn) ~= 1                  
      file = fgetl(fpn);          
      obj_origin = VideoReader(['video/',file(1:17),'.avi']);
      disp(['Exract Frame of ',file(11:17)]);
      detectionLabel = fopen(['DATA/train_Depth_DetectionLabel/',file(7:9),'/Label_',file(11:17),'.txt']);
