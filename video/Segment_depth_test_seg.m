clear
clc
fpn = fopen('../test_segmented.list');
ff_test_filename = fopen('test_seg_depth.txt','w');
...
while feof(fpn)~=1
    ...
    obj_depth = VideoReader(['../DATA/ConGD_phase_2/test/',Textfile{1}{1},'.K.avi']);
    ...
    for i = 2:num_text
        ...
        aviobj = VideoWriter(['test_seg/',num2str(base_name,'%05d'),'.K.avi']);
        ...
        fprintf(ff_test_filename,['test_seg/',num2str(base_name,'%05d'),'.K ',num2str(fnum),' ',frame{2},'\n']);
    end
end
fclose(ff_test_filename)
fclose(fpn)
