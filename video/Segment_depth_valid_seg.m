...
ff_valid_filename = fopen('valid_seg_depth.txt','w');
...
while feof(fpn)~=1
    ...
    obj_depth = VideoReader(['../DATA/ConGD_phase1/valid/',Textfile{1}{1},'.K.avi']);
    ...
    for i = 2:num_text
        ...
        aviobj = VideoWriter(['valid_seg/',num2str(base_name,'%05d'),'.K.avi']);
        ...
        fprintf(ff_valid_filename,['valid_seg/',num2str(base_name,'%05d'),'.K ',num2str(fnum),' ',frame{2},'\n']);
    end
end
