...
ff_train = fopen('train_rgb.txt','w');
base_name = 0;
while feof(fpn)~=1
    file = fgetl(fpn);
    Textfile = textscan(file,'%s');
    [num_text,n] = size(Textfile{1});
    obj_depth = VideoReader(['../DATA/ConGD_phase1/train/',Textfile{1}{1},'.M.avi']);
    ...
    for i = 2:num_text
        ...
        aviobj = VideoWriter(['train/',num2str(str2num(frame{2})-1,'%03d'),'/',num2str(base_name,'%05d'),'.M.avi']);
        ...
        fprintf(ff_train,['train/',num2str(str2num(frame{2})-1,'%03d'),'/',num2str(base_name,'%05d'),'.M ',num2str(fnum),' ',num2str(str2num(frame{2})-1),'\n']);
    end
end
fclose(ff_train)
fclose(fpn)       
