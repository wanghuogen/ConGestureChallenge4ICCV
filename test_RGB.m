...
fpn = fopen ('video/test_seg_rgb.txt');           %打开文档clear,clc

ff_train1 = fopen('Image4LSTM/test/test_rgb_fullbody.txt','w');
ff_train2 = fopen('Image4LSTM/test/test_rgb_full.txt','w');
...
while feof(fpn) ~= 1                %用于判断文件指针p在其所指的文件中的位置，如果到文件末，函数返回1，否则返回0  
      ...
      detectionLabel = fopen(['DATA/test_seg_Rgb_DetectionLabel/Label_',file(10:16),'.txt']);
      disp(['Exract Frame of ',file(10:16)]);
      ...
      outImageNamef = fullfile(Norm4S_dir1,file(10:16));
      outImageNamer = fullfile(Norm4S_dir2,file(10:16));
      if ~exist(fullfile(Norm4S_dir1,file(10:16)))
          mkdir(fullfile(Norm4S_dir1,file(10:16)));
      end
      if ~exist(fullfile(Norm4S_dir2,file(10:16)))
          mkdir(fullfile(Norm4S_dir2,file(10:16)));
      end
